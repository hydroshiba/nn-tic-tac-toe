...
if __name__ == "__main__":
	...
	model.load_state_dict(torch.load("model/mlp64.pth"))
	model.eval()

	minimax = agent.Minimax(depth=2, evaluation=model)

	for depth in range(1, 8):
		print(f"Evaluating Minimax2 with neural network (MLP 9 -> 64 -> 64 -> 1) against Minimax{depth} with pruning...")
		wins, draws, losses = evaluate(minimax, agent.Minimax(depth=depth, pruning=True), rounds=200)
